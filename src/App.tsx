...
function App() {
  ...
  const [currentPage, setCurrentPage] = useState<'main' | 'iso2' | 'docsdb' | 'document'>('main');
  ...
  const handleLogin = (user: User) => {
    setCurrentUser(user);
    // If user is admin, navigate to ISO2 page
    if (user.role === 'admin') {
      setCurrentPage('iso2');
    }
  };
  ...
  const handleDocumentSelect = (documentId: string, isSelected: boolean, event?: React.MouseEvent) => {
    if (!user) return;
  ...
  const handleBulkApproval = (action: 'approve' | 'reject') => {
    ...
    handleBulkAction('approve', Array.from(selectedDocuments));
  };
  ...
  const stats = {
    ...
    myDepartmentDocs: user ? filteredDocuments.filter(doc => doc.department === user.department).length : 0,
    ...
    return (
      <ThemeProvider>
        <LayoutComponent {...layoutProps}>
          <ISO2Page onNavigateToDocsDB={handleNavigateToDocsDB} />
        </LayoutComponent>
        <ThemeToggle />
      </ThemeProvider>
    );
  ...
  if (currentPage === 'docsdb') {
    return (
      <ThemeProvider>
        <LayoutComponent {...layoutProps}>
          <DocsDBPage onBack={handleBackToMain} />
        </LayoutComponent>
        <ThemeToggle />
      </ThemeProvider>
    );
              ...
              {user!.role === 'admin' && (
                <label className="flex items-center space-x-2 cursor-pointer">
                  <input
                    ...
                    className="rounded border-slate-300 dark:border-slate-600 text-eteal-600 shadow-sm focus:border-eteal-300 focus:ring focus:ring-eteal-200 focus:ring-opacity-50"
                  />
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div className="flex items-center space-x-2">
                  <button
                    onClick={() => handleBulkAction('approve', Array.from(selectedDocuments))}
                  ...
                  <button
                    onClick={() => handleBulkAction('delete', Array.from(selectedDocuments))}
                    className="glass-button text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700"
                  >
                    Delete All
                  </button>
                </div>
